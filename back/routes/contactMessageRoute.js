...
route.get('/messages', authMiddleware, (req, res) => {
  try {
    const data = readData(DATA_FILE);
    res.status(200).send(Array.isArray(data) ? data : []);
  } catch (error) {
...
route.delete('/message/:id', authMiddleware, (req, res) => {
  try {
    const { id } = req.params;
    const data = readData(DATA_FILE);
...
route.put('/message/:id/read', authMiddleware, (req, res) => {
  try {
    const { id } = req.params;
    const data = readData(DATA_FILE);
